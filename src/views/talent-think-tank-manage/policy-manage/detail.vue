...
export default {
  ...
  methods: {
    ...
    onEdit () {
      this.$router.push({ name: 'EditPolicy', query: { id: this.$route.query.id } })
    },
    getJourneyPolicyById () {
      const params = {
        id: this.$route.query.id
      }
      this.$api.getJourneyPolicyById(params).then(res => {
        if (res.isError) {
