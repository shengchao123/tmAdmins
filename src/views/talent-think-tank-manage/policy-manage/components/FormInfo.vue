  ...
  <div class="mt60">
    <el-form ref="form"
      ...
      <el-form-item label="发布单位"
        ...
        <span class="ft12 color-999 ml8">政策的实际发布单位，如区农业农村局，区政府，区卫生健康局等</span>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    // 根据id获取政策详情
    getJourneyPolicyById (id) {
      this.$api.getJourneyPolicyById({ id }).then(res => {
        if (res.isError) return this.$message.error(res.message)
  ...
  data () {
    return {
      actionMap: Object.freeze(new Map([
        [true, { apiName: 'modifyJourneyPolicy', msg: '修改' }],
        [false, { apiName: 'createJourneyPolicy', msg: '新增' }]
      ])),
      photoData: {
        imgs: '',
        index: 0
  ...
  created () {
    this.$route.query.id && this.getJourneyPolicyById(this.$route.query.id)
  }
}
</script>
