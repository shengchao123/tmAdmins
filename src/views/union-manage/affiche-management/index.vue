<template>
  <page title="公告管理">
    ...
    <el-button type="primary"
               @click="addACtivity"
               v-if="$hasAuth($urls.createJourneyAnnouncement)">新增公告</el-button>
    <page-table v-if="$hasAuth($urls.getJourneyAnnouncementPage)"
                :data="table.data"
      ...
      <el-table-column slot="operate"
        ...
        <template slot-scope="scope">
          <span class="text-btn mr16"
                ...
                v-if="$hasAuth($urls.getJourneyAnnouncementById)">详情</span>
          <el-popconfirm title="要删除该公告？"
                         cancelButtonType="取消"
                         confirmButtonText="删除"
                         hideIcon
                         @confirm="onDelete(scope.row)"
                         v-if="$hasAuth($urls.removeJourneyAnnouncement)">
...
import { afficheColConfig } from '@u/enum'
export default {
  name: 'index',
  data () {
    return {
  ...
  methods: {
    ...
    getTableData () {
      ...
      this.$api.getJourneyAnnouncementPage(params).then(res => {
    ...
    // 详情按钮
    onDetail (row) {
      this.$router.push({ name: 'AfficheDetail', query: { id: row.id } })
    },
    // 新增公告按钮
    addACtivity () {
      this.$router.push({ name: 'AddAffiche' })
    },
    // 删除按钮
    onDelete (row) {
      const params = {
      ...
      this.$api.removeJourneyAnnouncement(params).then(res => {
        if (res.isError) return this.$message.error(res.message)
